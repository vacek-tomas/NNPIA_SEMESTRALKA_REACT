...
class EditOdberatelComponent extends Component{
    ...
    componentDidMount() {
        OdberatelService.fetchOdberatelById(this.props.match.params.id)
            .then(res => {this.setState({...res.data.result})})
    }

    saveOdberatel = (e) => {
        e.preventDefault();
        OdberatelService.editOdberatel(this.props.match.params.id,{...this.state})
            .then(res => {
    ...
    render() {
        return(
            <Fragment>
                <Helmet>
                    <title>Úprava odběratele</title>
                </Helmet>
                <NavBar/>
                <Container>
                    <Typography variant="h4" style={style}>Úprava odběratele</Typography>
                    <form style={formContainer}>
                        ...
                        <Button variant="contained" color="primary" onClick={this.saveOdberatel}>Uložit změny</Button>
                    </form>
                </Container>
            </Fragment>
        );
...
export default EditOdberatelComponent;
